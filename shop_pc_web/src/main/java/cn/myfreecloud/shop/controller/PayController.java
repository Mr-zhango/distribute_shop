...
import cn.myfreecloud.shop.fegin.PayCallBackFegin;
...
public class PayController {
    ...
    public void synCallBack(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // 处理同步回调
        BaseResponse synCallBack = payCallBackFegin.synCallBack(params);

        if (!synCallBack.getResultCode().equals(Constants.HTTP_RES_CODE_200)) {
            return;
        }

        // 网络json(LinkedHashMap)
        LinkedHashMap data = (LinkedHashMap) synCallBack.getData();

        String htmlFrom = "<form name='punchout_form'"
                + " method='post' action='https://zfb.free.qydev.com/callBack/synSuccessPage' >"
